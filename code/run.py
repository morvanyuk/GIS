import psycopg
from connections import brocker, database
...
def add_restaurant(ch, method, properties, body):
    ...
    db.execute(f'''INSERT INTO Restaurants (restaurant_id, restaurant_name, coordinate) VALUES ({data.restaurant_id}, '{data.restaurant_name}', ST_MakePoint({data.coordinate.lat}, {data.coordinate.long}))''')
    db_commit()
...
def remove_restaurant(ch, method, properties, body):
    data = int(body)
    # Checking permissions
    db.execute(f'''DELETE FROM restaurants WHERE restaurant_id={data}''')
    db_commit()
    SendingForOwner(properties=properties, body=200)
...
def get_restaurant(ch, method, properties, body):
    data = int(body)
    # Checking permissions
    result = db.execute(f'''SELECT ST_X(coordinate), ST_Y(coordinate) FROM restaurants WHERE restaurant_id={data}''').fetchone()
...
def add_order(ch, method, properties, body):
    ...
    db.execute(f'''INSERT INTO Orders (coordinate, id, restaurant) VALUES (ST_MakePoint({data.coordinate.lat}, {data.coordinate.long}), {data.object_id}, {data.restaurant_id})''')
    db_commit()
    SendingForOwner(properties=properties, body=200)
...
def remove_order(ch, method, properties, body):
    data = int(body)
    db.execute(f'''DELETE FROM orders WHERE id={data}''')
    db_commit()
    SendingForOwner(properties=properties, body=200)
...
def in_radius(ch, method, properties, body):
    ...
    result = list(db.execute(f'''SELECT restaurant_id FROM restaurants WHERE ST_DWithin(coordinate, ST_MakePoint({data.lat}, {data.long}), 15000, TRUE)''').fetchall())
    if len(result) > 0:
        for i in result:
